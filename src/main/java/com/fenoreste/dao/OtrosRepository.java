...
public interface OtrosRepository extends CrudRepository<UsuariosBancaMovil, Long> {
	...
	@Query(value = " SELECT count(*) FROM sopar WHERE idorigen= ?1 AND idgrupo = ?2 AND idsocio = ?3 AND tipo = ?4", nativeQuery = true)
	...
	@Query(value ="SELECT update_encoding(?1,?2,?3,?4)" , nativeQuery = true)
    void update_encoding(String curp,Integer idorigen,Integer idgrupo,Integer idsocio);
