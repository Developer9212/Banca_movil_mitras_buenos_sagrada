...
public interface AmortizacionesRepository extends JpaRepository<Amortizacion, Integer> {
	...
	@Query(value = "SELECT vence,idamortizacion,abono,abonopag,todopag,atiempo,diasvencidos,io FROM amortizaciones WHERE idorigenp=?1 AND idproducto=?2 AND idauxiliar=?3 " , nativeQuery = true)
	...
	//Lista de pagos
	@Query(value = "SELECT vence,idamortizacion,abono,abonopag,todopag,atiempo,diasvencidos,io FROM amortizaciones WHERE idorigenp=?1 AND idproducto=?2 AND idauxiliar=?3 ORDER BY idamortizacion" , nativeQuery = true)
