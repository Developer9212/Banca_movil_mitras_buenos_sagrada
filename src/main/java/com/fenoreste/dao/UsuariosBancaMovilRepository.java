...
public interface UsuariosBancaMovilRepository extends JpaRepository<UsuariosBancaMovil, Long> {	
	...
	@Query(value = " SELECT * FROM banca_movil_usuarios_bankingly WHERE idorigen= ?1 AND idgrupo = ?2 AND idsocio = ?3", nativeQuery = true)
	UsuariosBancaMovil findByOgs(Integer idorigen, Integer idgrupo, Integer idsocio);

	@Transactional
	@Modifying
	@Query(value = " INSERT INTO banca_movil_usuarios_bankingly VALUES (?1,?2,?3,?4,?5,?6,?7,?8,(SELECT fechatrabajo FROM origenes LIMIT 1))", nativeQuery = true)
	...
	@Query(value = " SELECT * FROM banca_movil_usuarios_bankingly WHERE alias_usuario=?1", nativeQuery = true)
	UsuariosBancaMovil findByUsername(String usuario);
