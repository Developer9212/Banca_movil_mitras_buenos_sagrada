...
import com.fenoreste.modelos.ByDocumentsDTO;
...
public class CustomerServiceSpring {
	...
	public ByDocumentsDTO getClientsByDocuments(String name, String lastName, String documentId, String mail,
		...
		Persona p = personaService.findPersonaMatriculacionJdbc(name.toUpperCase().replace(" ", "").trim(),
		...
		if (p != null) {
			...
			if (tb_producto_banca_movil != null) {
				...
				a_producto_banca_movil = auxiliaresService.findAuxiliarBancaMovil(p.getIdorigen(), p.getIdgrupo(),
						p.getIdsocio(), Integer.parseInt(tb_producto_banca_movil.getDato1()));
				if (a_producto_banca_movil != null) {
					// busco si no existe un registro para el usuario
					UsuariosBancaMovil userExists = usuariosService.buscarUsuarioPorNombreUsuario(username);
					if (userExists == null) {						
						// CSN
						...
						// Buenos aires
							...
							if (validacion_buenos(p).toUpperCase().contains("EXITO")) {
								bandera_validacion = true;
							} else {
								matriculacion.setClientName(validacion_mitras(p).toUpperCase());
							}
						// Sagrada Familia
						} else if(origen.getIdorigen() == 20700) {
							if(validacion_sagrada(p).toUpperCase().contains("EXITO")){
								bandera_validacion = true;
							}else {								
								matriculacion.setClientName(validacion_sagrada(p).toUpperCase());
							}
						}
					} else {
						if (userExists.getIdorigen().intValue() == p.getIdorigen().intValue() && userExists.getIdgrupo().intValue() == p.getIdgrupo().intValue()
								&& userExists.getIdsocio().intValue() == p.getIdsocio().intValue()) {
							// Mitras
							...
							matriculacion.setClientName("YA EXISTE UN USUARIO CON EL MISMO NOMBRE");
						}
					}
				} else {
		...
		if (bandera_validacion) {
			...
			// if(usuario != null) {
			// matriculacion.setClientName("USUARIO YA ESTA REGISTRADO CON EL
			// ALIAS:"+usuario.getAlias_usuario());
			// }else {
			...
			matriculacion.setClientBankIdentifier(String.format("%06d", p.getIdorigen())
					+ String.format("%02d", p.getIdgrupo()) + String.format("%06d", p.getIdsocio()));
			if (usuario == null) {
				...
				usuariosService.save(usuario.getIdorigen(), usuario.getIdgrupo(), usuario.getIdsocio(), username,
						usuario.getIdorigenp(), usuario.getIdproducto(), usuario.getIdauxiliar(), true);
			} else {
		...
		if (tb_producto_tdd != null) {
			...
			if (a_producto_tdd != null) {
				...
				if (a_producto_tdd.getEstatus() == 2) {
					...
					//try {
						BalanceQuery = null; // new JSONObject(httpService.getBalanceQuery(folioTdd.getIdtarjeta()));
	...
	public String validacion_mitras(Persona p) {
		String mensaje_validacion = "";		
		if(p.getIdgrupo() == 10) {
			//Buscando si ya existe un registro para el socio
			if(otrosService.validaciones_sopar_bloqueo(p.getIdorigen(),p.getIdgrupo(),p.getIdsocio())) {
				mensaje_validacion = "SOCIO BLOQUEADO";
			}else if(otrosService.validaciones_sopar_cancelacion(p.getIdorigen(),p.getIdgrupo(),p.getIdsocio())) {
				mensaje_validacion = "CUENTA CANCELADA";
			}else {
		   	   mensaje_validacion = "VALIDADO CON EXITO";
			}
	...
	public String validacion_buenos(Persona p) {
		String mensaje_validacion = "";		
		if(p.getIdgrupo() == 10) {
			if(otrosService.validaciones_sopar_bloqueo(p.getIdorigen(),p.getIdgrupo(),p.getIdsocio())) {
				mensaje_validacion = "SOCIO BLOQUEADO";
	...
	public String validacion_sagrada(Persona p) {
		String mensaje_validacion = "";		
		if(p.getIdgrupo() == 10) {
			if(otrosService.validaciones_sopar_bloqueo(p.getIdorigen(),p.getIdgrupo(),p.getIdsocio())) {
				mensaje_validacion = "SOCIO BLOQUEADO";
