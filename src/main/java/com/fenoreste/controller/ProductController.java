...
import com.fenoreste.modelos.ProductBankStatementDTO;
import com.fenoreste.modelos.ProductConsolidatedPositionDTO;
import com.fenoreste.modelos.ProductosDTO;
...
import com.fenoreste.util.EliminarArchivosThread;
...
public class ProductController {
	...
	public ResponseEntity getProducts(@RequestBody String cadena) {
		...
		//Ejecutamos el hilo para eliminar archivos TXT,HTML,PDF(estados de cuenta,voucher)
		EliminarArchivosThread eliminacion = new EliminarArchivosThread();
		eliminacion.start();
		...
		//Validacion para MITRAS
		if(origen.getIdorigen() == 30300) {
			ogsDTO ogs = util.ogs(ClientBankIdentifiers); 
			//MITRAS validamos si el socio esta bloqueado en banca movil
			...
			}else if(otrosService.validaciones_sopar_cancelacion(ogs.getIdorigen(), ogs.getIdgrupo(), ogs.getIdsocio())) {
				JsonObject json_bloqueo=new JsonObject();
				json_bloqueo.put("Error", "CUENTA CANCELADA");
				return ResponseEntity.status(400).body(json_bloqueo);
			}
		}
	...
	public ResponseEntity getBankStatements(@RequestBody String cadena) {
		...
		try {
	        ...
	        List<ProductBankStatementDTO> listaECuentas = productosServiceSpring.bankStatements(productBankIdentifier_, productType_);
			...
			System.out.println("Error al obtener estados de cuenta controller:"+e.getMessage());
		}
